...
import { rebaseSubtreePath } from "../src/performance/incremental-updater.js";
...
describe("subtree path rebasing", () => {
  it("prefixes descendant paths after reparsing a nested subtree", () => {
    const { root } = parseDocument('[{"gtinList":["00670983139990"]}]', "small");

    rebaseSubtreePath(root, ["products"], "products");

    expect(root.path).toEqual(["products"]);
    expect(root.children![0].path).toEqual(["products", 0]);
    expect(root.children![0].children![0].path).toEqual(["products", 0, "gtinList"]);
    expect(root.children![0].children![0].children![0].path).toEqual(["products", 0, "gtinList", 0]);
  });
});
