...
import type { ParseTier } from "../src/utils/size-guard.js";
import type { ParsedDocumentState } from "../src/core/document-store.js";

function makeState(uri: string, json: string, tier: ParseTier = "small"): ParsedDocumentState {
  const { root, errors } = parseDocument(json, tier);
  return {
    uri,
    version: 1,
    root,
...
describe("DocumentStore", () => {
  ...
  describe("getNodeAtOffset", () => {
    ...
    it("loads nested children to find the deepest node at offset", () => {
      const text = '{"images": [{"l": "712k6Dd3geL.jpg", "w": 2560}]}';
      store.set("file:///a.json", makeState("file:///a.json", text, "medium"));

      const offset = text.indexOf('"712k6Dd3geL.jpg"') + 1;
      const node = store.getNodeAtOffset("file:///a.json", offset);

      expect(node?.path).toEqual(["images", 0, "l"]);
      expect(node?.value).toBe("712k6Dd3geL.jpg");
    });
