...
export function registerCommands(
  ...
  ctx.subscriptions.push(
    vscode.commands.registerCommand("jsonlens.copyValue", (arg?: unknown) => {
      const node = getCommandNode(arg);
      if (!node) return;
    ...
    vscode.commands.registerCommand("jsonlens.openPreview", (arg?: unknown) => {
      openPreview(arg, editorProvider);
    }),
    ...
    vscode.commands.registerCommand("jsonlens.openInJsonLens", async () => {
      ...
      if (uris?.[0]) {
        openPreview(uris[0], editorProvider);
      }
    }),

    vscode.commands.registerCommand("jsonlens.openWithPreview", (arg?: unknown) => {
      openPreview(arg, editorProvider);
    }),
...
function openPreview(arg: unknown, editorProvider: JsonLensEditorProvider): void {
  let uri: vscode.Uri | undefined;

  if (arg instanceof vscode.Uri) {
    uri = arg;
  } else if (arg && typeof arg === "object" && "uri" in arg && arg.uri instanceof vscode.Uri) {
    uri = arg.uri;
  } else {
    const activeUri = getActiveJsonUri(editorProvider);
    if (activeUri) uri = vscode.Uri.parse(activeUri);
  }
  ...
  const target = getCommandNode(arg) ?? getNodeAtCursor();
  if (target) {
    // If the target node is from a different file, don't send it to this preview
    ...
    setTimeout(() => {
      if (editorProvider.panels.has(uriStr)) {
        editorProvider.send(uriStr, {
          type: "node.selected",
          payload: buildNodePayload(target),
          columns: buildNodePayloadTrail(target, (path) => documentStore.getNodeAtPath(uriStr, path)),
        });
      }
    }, 500);
  }
...
function getCommandNode(arg: unknown): JsonNode | undefined {
  return isJsonNode(arg) ? arg : undefined;
}

function isJsonNode(value: unknown): value is JsonNode {
  if (!value || typeof value !== "object") return false;
  const node = value as Partial<JsonNode>;
