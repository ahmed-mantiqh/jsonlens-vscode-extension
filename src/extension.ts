...
export function activate(ctx: vscode.ExtensionContext): void {
  ...
  const updateContext = () => {
    ...
    const isJson = editor && SUPPORTED.has(editor.document.languageId);
    const isCustomActive = !!editorProvider.activeUri;
    vscode.commands.executeCommand("setContext", "jsonlens.active", isJson || isCustomActive);
  };

  ctx.subscriptions.push(vscode.window.onDidChangeActiveTextEditor(updateContext));
  ctx.subscriptions.push(vscode.window.tabGroups.onDidChangeTabs(updateContext));
  updateContext();
